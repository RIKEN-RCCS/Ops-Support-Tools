...
- 通常の AI パイプラインで Zendesk に書き込むのは `poster.py` だけです。
- 通常の AI パイプラインの書き込み操作は内部メモ追加、タグ追加、任意のカスタムフィールド更新に限定しています。
- 例外として、`/zendesk/webhook/send_internal_note` は人間が作った最新社内メモを AI なしで公開返信へ転送します。
...
| `SUPPORT_AI_SEND_INTERNAL_NOTE_MARKER` | 最新社内メモを公開返信へ転送する先頭行 marker。既定 `[SEND_INTERNAL_NOTE]` |
| `SUPPORT_AI_INTERNAL_NOTE_SENT_TAG` | 最新社内メモを公開返信へ転送したときに付ける Zendesk tag。既定 `internal_note_sent` |
| `SUPPORT_AI_SEND_INTERNAL_NOTE_MAX_LEN` | 最新社内メモを公開返信へ転送する最大本文長。既定 `30000` |
...
社内メモを人間が確認済みの公開返信文として使う場合は、AI を通さず最新の社内メモをそのままエンドユーザー向け公開返信へ転送する専用 webhook を使えます。

```text
POST https://your-host.example.com/zendesk/webhook/send_internal_note
```

この endpoint は queue や LLM を通しません。Ticket Comments API で最新の `public=false` comment を取得し、先頭行の `SUPPORT_AI_SEND_INTERNAL_NOTE_MARKER`、既定では `[SEND_INTERNAL_NOTE]`、を削除した本文を Ticket Update API の `public=true` comment として投稿します。投稿後は既存タグを保持したまま `SUPPORT_AI_INTERNAL_NOTE_SENT_TAG`、既定では `internal_note_sent`、を追加します。

Zendesk trigger は社内メモ本文が `[SEND_INTERNAL_NOTE]` を含む、または先頭行が `[SEND_INTERNAL_NOTE]` になる条件で発火させます。アプリ側は最新社内メモの先頭行が marker と完全一致する場合だけ送信します。marker は公開返信本文から削除されます。
...
payload は通常の webhook と同じく `ticket_id` だけで動きます。

```json
{"ticket_id": 12345}
```

Zendesk 側では、送信したい社内メモが必ず最新の社内メモになる条件で trigger を作ってください。最新社内メモの先頭行に marker がない、本文が空、最新社内メモがない、または `SUPPORT_AI_SEND_INTERNAL_NOTE_MAX_LEN` を超える場合は公開返信せずエラーを返します。
