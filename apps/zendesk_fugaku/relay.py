...
class Config:
    ...
    def from_env(cls) -> "Config":
        scopes_raw = os.environ.get("ZENDESK_FUGAKU_OAUTH_SCOPES", "read write")
        scopes = [item.strip() for item in scopes_raw.replace(",", " ").split() if item.strip()]
        data_dir = Path(os.environ.get("ZENDESK_FUGAKU_DATA_DIR", "/data"))
        return cls(
            zendesk_url=os.environ.get("ZENDESK_FUGAKU_URL", "https://fugaku.zendesk.com").rstrip("/"),
            client_identifier=os.environ.get("ZENDESK_FUGAKU_OAUTH_CLIENT_IDENTIFIER", "").strip(),
            client_secret=read_secret("ZENDESK_FUGAKU_OAUTH_CLIENT_SECRET"),
            token_bundle_file=Path(os.environ.get(
                "ZENDESK_FUGAKU_OAUTH_TOKEN_BUNDLE_FILE",
    ...
    def validate(self) -> None:
        ...
        if not self.zendesk_url:
            missing.append("ZENDESK_FUGAKU_URL")
...
def main() -> int:
    ...
    thread.start()
    log(f"starting zendesk_fugaku relay for {config.zendesk_url}")
