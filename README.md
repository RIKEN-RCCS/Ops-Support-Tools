...
Compose は `zendesk-support-ai` の各 worker と `knowledge-api` を起動します。`OPS_SUPPORT_TOOLS_TZ` は設置サーバのローカルタイムゾーンに合わせてください。詳細は [`apps/zendesk-support-ai/README.md`](apps/zendesk-support-ai/README.md) と [`apps/knowledge-api/README.md`](apps/knowledge-api/README.md) を参照してください。
